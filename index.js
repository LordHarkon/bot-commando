...
client
    ...
    .on('commandError', (cmd, err) =>
        require('./events/commandError')(cmd, err)
    );
...
client.on('commandRun', async (cmd, promise, msg) => {
    if (msg.channel.type === 'dm') {
        ...
        cmds.info(
            `${msg.author.tag} (${
                msg.author.id
            }) ran the command ${cmd.name.toUpperCase()} in ${
                msg.guild.name
            } (${msg.guild.id})`
        );
    }
...
client.on('guildMemberRemove', async (member) => {
    ...
    const membersChannel = channels.find(
        (x) => x.id === process.env.MEMBERCOUNTCHANNEL
    );
...
client.on('messageUpdate', async (oldMessage, newMessage) => {
    ...
    const logs = oldMessage.channel.guild.channels.find(
        (x) => x.name === process.env.LOGS
    );
