...
const { balance, get } = require('../../util/bank');
...
module.exports = class BankCommand extends Command {
    ...
    run(msg, { user }) {
        const money = balance(user.id)
        const loan = stripIndent`
            **Money to pay (loan):** __${get(user.id, 'loan')}__
            **Interest rate:** __${get(user.id, 'interest')}%/day__
            **Time to pay:** __${(30-(get(user.id, 'date'))) < 0 ? 0 : 30-(get(user.id, 'date'))} Days__
        `;
        var rank = '';
